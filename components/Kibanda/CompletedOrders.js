...
{
  /*
  there is no way mtu akawa ndo mwenye orders zimewekwa kwake afu aka-reject mtu 
  mwingine, so there is no way to check who rejected order because its you who order
  placed to so in any case it should be you who should be able to reject the order
*/
}
...
function CompletedOrders() {
  ...
  const [kibandaOrders, setKibandaOrders] = useState(
    AppCtx.kibandaOrders.filter(
      (order) =>
        order.order_status.toLowerCase() === "Completed".toLowerCase() &&
  ...
  useEffect(() => {
    if (AppCtx.activeKibandaOrderMetadata.tab) {
      ...
      if (shouldRefresh && tab === "Completed Orders") {
        fetchOrders();
  ...
  async function deleteOrderHandler(order_id) {
    ...
    try {
      const order = await kibandaMarkOrderDeleted(order_id);
      setMessage("Success");
      setIcon("check");
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      alert("Connection Error, check your intenet")
    }
    setIsLoading(false);
  };
